...
<body>
  <header>
      ...
      <hr>
  <h2>THEME 게시판 글 보기</h2>
  
  	<div>
  		<a href="warning?userNickname=${ThemePageVO.userNickname }">
  ...
  <div>
    <p>
	      ...
	      제목 : ${ThemePageVO.themaTitle }
	   
    </p>
  </div>
  <div>
    <p>작성자 : ${ThemePageVO.userNickname }</p>
    <fmt:formatDate value="${ThemePageVO.themaCdate }" pattern="yyyy-MM-dd HH:mm:ss"
      var="themaCdate" />
    <p>작성일 : ${themaCdate }</p>
    <p>조회수 : ${ThemePageVO.themaViewCnt }</p>
    
    <div id="like">
    	<p>좋아요 : ${ThemePageVO.themaGoodCnt }</p>
    </div>
    ...
    <div id="wish">
	    <p>찜 : ${ThemePageVO.themaWishCnt }</p>
	</div>
  ...
  <div class="inputArea">
 		<label for="img"></label>
		 <img src="display?fileName=${ThemePageVO.img}" class="oriImg"/>
		 
	</div>
	
	<div>
    	<textarea rows="20" cols="120" readonly="readonly">${ThemePageVO.themaContent }</textarea>
  	</div>
  	...
  	<c:if test="${not empty custom.username }">
	  	...
	  	<c:if test="${custom.uvo.userNickName == ThemePageVO.userNickname}">
			<div>
				<a href="list?page=${page }"><input style="background-color:#4CAF50;" type="button" value="글 목록"></a>
			    <a href="update?themeNo=${ThemePageVO.themaNo }&page=${page}"><input style="background-color:#4CAF50;" type="button" value="글 수정"></a> 
			    <a href="delete?themeNo=${ThemePageVO.themaNo }"><input style="background-color:#4CAF50;" type="button" value="글 삭제"></a>
	...
	<div style="text-align: left;">
      <div>
        <input type="hidden" id="themaNo" name="themaNo" value="${ThemePageVO.themaNo }">
        <c:if test="${custom.username == custom.uvo.userID}">
        ...
        <input type="text" id="themaReviewContent" name="themaReviewContent" placeholder="댓글 입력" required>
 ...
 <script type="text/javascript">
    $(document).ready(function () {
        ...
        var goodCnt = ${ThemePageVO.themaGoodCnt };
        var wishCnt = ${ThemePageVO.themaWishCnt };
        ...
        $(".heart").on("click", function () {
            ...
            var sendData = {'themaNo' : themaNo,'heart' : that.prop('name'), 'nickName' : nickName};
        ...
        $(".basket").on("click", function () {
            ...
            var sendData = {'themaNo' : themaNo,'basket' : that.prop('name'), 'nickName' : nickName};
            var csrfHeaderName = "${_csrf.headerName}";
    		var csrfTokenValue = "${_csrf.token}";
    		console.log(sendData);
        ...
        $('#btn_add').click(function(){
	     ...
	     // 게시판의 댓글 전체 가져오기
	  	 function getAllReplies()
	  	  {
	  	    var url = '../theme/replies/all/' + themaNo ;
	  	    $.getJSON(
	  	    	url,
	  	    	function(jsonData){
	  	    	  ...
	  	    	  {
	  	    	    var themaReviewCdate = new Date(this.themaReviewCdate);
		          	...
		          	console.log(disabled);
	  	          	...
	  	          	list += '<div class="reply_item">'
	                       + '<pre>'
	                       + '<input type="hidden" id="themaReviewNO" value="' + this.themaReviewNO + '" />'
	                       + '<input type="hidden" id="userNickname" value="' + this.userNickname + '" />'
	                       + this.userNickname
	                       + '&nbsp;&nbsp;' // 공백
	                       + '<input type="hidden" id="themaReviewContent" value="' + this.themaReviewContent + '" />'
	                       + this.themaReviewContent
	                       + '&nbsp;&nbsp;' // 공백
	                       + themaReviewCdate
	                       + '&nbsp;&nbsp;' // 공백
	      	...
	      	$('#replies').on('click', '.reply_item .btn_update',
	              function() {
	                ...
	                var reviewNo = $(this).prevAll('#themaReviewNO').val();
	                var reviewContent = $(this).prevAll('#themaReviewContent').val();
	             ...
	             // ajaxSetup
	                ...
	                $.ajax({
	                  ...
	                  data : JSON.stringify({
	                    ...
	                    'themaReviewContent' : updateContent,
	                    'themaNo' : themaNo,
	                    'userNickname' : userNickName,
	                    'themaReviewNO' : reviewNo
         ...
         $('#replies').on('click', '.reply_item .btn_delete',
             function() {
               ...
               // 선택된 댓글 replyNo
               var themaReviewNo = $(this).prevAll('#themaReviewNO').val();
               var themaNo = $('#themaNo').val();
               console.log("선택된 댓글 번호 : " + themaReviewNo);
               ...
               $.ajax({
                 type : 'delete',
                 url : '../theme/replies/' + themeReviewNo,
                 headers : {
                   'Content-Type' : 'application/json',
